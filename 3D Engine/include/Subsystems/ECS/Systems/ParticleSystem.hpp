...
private:
    ...
    void spawn_particle(Components::ParticleEmitter& emitter, Components::Transform const& abs_transform);
	...
	glm::vec3 position_on_circle(float radius, float arc, glm::vec3 const& rotation);
	glm::vec3 direction_in_cone(float arc, float angle, glm::vec3 const& rotation);

	// scale specifies how to scale the box centered (0, 0, 0) with size(1, 1, 1)
