...
void main() {
    ...
    for(int i = 0; i < point_light_count; ++i) {
        // ambient lighting
        light_result += point_lights[i].ambient * vec3(texture(material.diffuse_map, TexCoords));
        // diffuse lighting
        vec3 light_dir = normalize(point_lights[i].position - FragPos);
        float diff = max(dot(norm, light_dir), 0.0);
        vec3 diffuse =  point_lights[i].diffuse * (diff * vec3(texture(material.diffuse_map, TexCoords)));
        light_result += diffuse;
        // specular lighting
        vec3 view_dir = normalize(camera_position - FragPos);
        vec3 reflect_dir = reflect(-light_dir, norm);
        float spec = pow(max(dot(view_dir, reflect_dir), 0.0), material.shininess);
        vec3 specular = point_lights[i].specular * (spec * vec3(texture(material.specular_map, TexCoords)));
